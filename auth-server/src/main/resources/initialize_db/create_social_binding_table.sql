...
-- so login resolution is sub-first. ON DELETE CASCADE so deleting a credential
-- (e.g. account-management's DELETE /api/account/test, which knows nothing about
-- this table) takes its bindings with it.
...
CREATE TABLE social_binding (
    ...
    PRIMARY KEY (id),
    UNIQUE (social_provider, social_user_id),
