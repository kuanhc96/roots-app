...
| `PUT /api/account/password/{userGUID}` | none (public) | Updates `password` from body `{ "password": "..." }` and returns `{ "userGUID" }`. Password policy matches auth-server (`>=8`, at least one uppercase, one lowercase, one number). |
| `PUT /api/account/name/{userGUID}` | none (public) | Updates `name` from body `{ "name": "..." }` and returns `{ "userGUID", "name" }`. Input is trimmed; trimmed value must be non-blank and at most 255 chars. |
| `PUT /api/account/email/{userGUID}` | none (public) | Updates `email` from body `{ "email": "..." }` and returns `{ "userGUID", "email" }`. Input is trimmed and validated (`contains '@'`, non-blank); duplicate email returns **409**. |
| `POST /api/account/role/{userGUID}` | none (public) | Adds a role from body `{ "role": "..." }` to the account and returns `{ "userGUID", "roles": [...] }` (all roles after the operation). Returns **201** if the role was added, **200** if the account already had it. Returns **400** for an invalid or GUEST role. |
