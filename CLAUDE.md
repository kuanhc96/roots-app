...
3. The endpoint **always returns 200** regardless of whether the email exists — account existence is never revealed (no enumeration). `ForgotPasswordForm` therefore ignores the outcome and (in a `finally`) `navigateTo({ path: '/login', query: { email, notice: 'tempPasswordSent' } })`. Because that's in-SPA client navigation (not a full reload), the query survives hydration; `LoginForm.vue` reads it via `useRoute()` to **pre-fill the email** and show a success `v-snackbar`: "If the email provided matches one we have on file, you will receive an email with a temporary password."
...
| `SpaController.forwardResetPassword` / `resetPassword` (`controller/`) | `GET`/`POST /reset-password` — serve the form; verify the pending token, store the new password, clear the flag, verify email, upgrade session, redirect to saved request |
...
| `LoginIntegrationTest` | Form-login happy path: creates an account with default roles, MFA off, email verified, no password change required; starts `/oauth2/authorize`; `POST /login` (asserting the immediate 302 returns to the saved authorize request, not a pending step); follows the chain to the callback; exchanges the code; asserts `sub` = the account email and `roles` contains `MEMBER` |
...
**Per-test account creation in `LoginIntegrationTest`:** unlike `ForgotPasswordIntegrationTest` (which creates its account in `@BeforeEach` because every test wants the same shape), `LoginIntegrationTest` deliberately has **no** account-creating `@BeforeEach`: each test creates its own account variant inline and records the `userGUID` into the class field, so future tests with different flag combinations (MFA enabled, unverified email, …) can share the class. Its `@AfterEach` delete is therefore null-guarded — a test that fails before creating its account has nothing to clean up.
