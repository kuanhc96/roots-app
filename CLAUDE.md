...
The `auth-server-db` MySQL instance runs on port **3308** (not the default 3306) and is defined in `docker-compose.yml`. DB schema is in `auth-server/src/main/resources/initialize_db/`.
...
**auth-server sessions are Redis-backed (Spring Session).** The browser session cookie is `__Host-AUTH_SESSION` (Secure, HttpOnly, `Path=/`, `SameSite=Lax`), backed by `auth-server-redis` (defaults: `REDIS_HOST=localhost`, `REDIS_PORT=6381`). Servlet-container tracking modes are disabled (`ServletContainerSessionConfig`), so Tomcat does not emit `JSESSIONID` or URL-rewritten session ids.
...
│   │   └── RoleApiCard.vue              # wide card with 6 role API buttons + response display; authorize button triggers OAuth2 Authorization Code flow if no valid access token; logout button calls useOAuth's startLogout() — a full navigation to bff-server's GET /api/auth/logout, which clears the Redis tokens and drives OIDC RP-Initiated Logout server-side (the /logout landing page then clears sessionStorage)
│   ├── composables/
...
**`GET /api/auth/logout`** (`AuthController` → `LogoutService`): server-side RP-Initiated Logout. Reads the session's id_token (for the hint) **before** clearing, then `TokenStoreService.clearTokens` deletes the session's three token keys (`access_token`/`id_token`/`refresh_token`; `oauth_state` is left alone — it only exists mid-authorize-flow), and the controller `session.invalidate()`s so the `__Host-SESSION` cookie and its `spring:session:sessions:*` entry are dropped (a fresh session is minted next request under `ALWAYS`). Returns an unconditional **302** to `{auth-server.external-location}/connect/logout?client_id={web.client.id}&post_logout_redirect_uri={web.client.origin}/logout` with `id_token_hint=<id_token>` appended **only when** an id_token is held. `client_id` is sent **always** so auth-server can identify the client and honour `post_logout_redirect_uri` even without a hint (a refresh-only-revived login never stored an id_token); when the hint is present, Spring's OIDC logout skips its confirmation page and 302s straight through. External-location for the same browser-follows-this reason as authorize. The browser can't send `id_token_hint` itself — it never holds the id_token — which is the whole reason logout is server-side. web-client's `useOAuth.ts` splits the two concerns: `startLogout()` does a full navigation to this endpoint (wired to the logout button); the `/logout` landing page (the `post_logout_redirect_uri`) runs only the local `logout()` that clears the stored claims — never re-triggering the server flow, so the post-logout landing can't loop.
...
- **Spring Security** (`config/SecurityConfig.java`): `anyRequest().permitAll()`; CSRF disabled (project convention — revisit when state-changing BFF endpoints land, since the `__Host-SESSION` cookie will be the browser's only credential); `SessionCreationPolicy.ALWAYS` — every request eagerly gets a session, because the session is where tokens will be held.
- **Spring Session Redis** (`spring-boot-starter-session-data-redis`): HTTP sessions live in Redis (`spring:session:sessions:*` keys); the only session cookie is `__Host-SESSION` (container tracking modes are disabled, so Tomcat does not emit `JSESSIONID`). **Boot 4 gotcha:** session auto-config ships in Boot's own `spring-boot-session-data-redis` module — depending on the plain `org.springframework.session:spring-session-data-redis` jar alone silently leaves the in-memory container session in place; use the starter.
- **CORS**: only `web.client.origin` (default `http://localhost:3000`, override `WEB_CLIENT_ORIGIN`) is allowed, with `allowCredentials=true` so the browser can send the `__Host-SESSION` cookie cross-origin; a preflight from any other origin gets 403.
...
# Start the Redis instance backing auth-server sessions/state (no env vars needed)
...
Auth-server DB schema (MySQL 8, port 3308). The `account-management` service shares this same DB and the `user_credential`/`role` tables (it has no schema of its own):
