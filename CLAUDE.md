...
| Class | Role |
|---|---|
| `TempPasswordController` (`controller/`) | `POST /api/temp-password` — always 200; on match, generate+persist+email a temp password |
...
4. Starts auth-server in the background with `java -jar`, under `SPRING_PROFILES_ACTIVE=test` (so `emailSender.enabled=false` — no real emails). Real `SPRING_MAIL_*` secrets are still supplied because the `JavaMailSender` is built in every profile and the Actuator mail health indicator connects to SMTP during the health poll.
5. Polls `GET /actuator/health` until `UP` (150 s timeout).
6. Runs integration tests with `mvn surefire:test`.

**Required GitHub secrets:** `MYSQL_AUTH_SERVER_ROOT_USERNAME` (set to `root`), `MYSQL_AUTH_SERVER_ROOT_PASSWORD`, and real `SPRING_MAIL_USERNAME`/`SPRING_MAIL_PASSWORD` (Gmail address + App Password). The mail secrets are required even though the `test` profile sends no email: the `JavaMailSender` is built in every profile and the Actuator mail health indicator opens an SMTP connection on each `/actuator/health` poll, so invalid creds would fail the healthcheck step.
